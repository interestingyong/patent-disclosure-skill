...
产出形态为**交底稿**时（默认）：无论类型，另须过 **§8.6**；产出形态为申请文件时按 patent-application 门禁，§8.6 的红线项不适用。
...
## 8.6 交底稿形态核查（产出形态为交底稿时必做）

- [ ] **权项句式**：正文**未**出现权利要求书式条款（如「一种……，其特征在于……」独权句式）
- [ ] **FTO / 侵权分析**：**未**出现自由实施（FTO）、侵权比对或规避设计讨论
- [ ] **上位化讨论**：**未**出现上位化 / 概括策略分析（属代理人阶段工作）
- [ ] **三节齐备**：方案内容、创新点、差异点三节齐备，创新点逐条可回指方案内容
- [ ] **图题一致**：每个图有图题，图题与图内容一致，图内文字不超框、风格统一、图像清晰
- [ ] **数据口径**：实验数据均有口径备注（样本/构建号/日期/环境）
